...
class DetailViewModel: DetailViewModelProtocol, ErrorPresentable {
    ...
    @Inject private var mediaItemStore: MediaItemStoreProtocol
    ...
    init() {
        ...
        let credits = mediaItemSubject
            .flatMap { [weak self] mediaItem in
                ...
                return mediaItem.type == .tv ? self.repository.fetchTVCredits(req: request)
        ...
        let reviews = mediaItemSubject
            .flatMap { [weak self] mediaItem in
                ...
                return mediaItem.type == .tv ? self.repository.fetchTVReviews(req: request)
        ...
        let similars = mediaItemSubject
            .flatMap { [weak self] mediaItem in
                guard let self = self else { preconditionFailure("There is no self") }
                let request = FetchSimilarMoviesRequest(movieId: mediaItem.id)
        ...
        Publishers.CombineLatest4(details, credits, reviews, similars)
            ...
            } receiveValue: { [weak self] details, credits, reviews, similar in
                guard let self = self else { preconditionFailure("There is no self") }
                self.mediaItemDetail = details
                self.credits = credits
                self.reviews = reviews.prefix(4).map { $0 }
                self.similarMovies = similar.mediaItems
